...
export default function PortalCharts({ clientSlug }: PortalChartsProps) {
  ...
  const fetchChartsData = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setError(errorData.error || 'Erro ao carregar dados dos gráficos')
      }
    } catch (err) {
  ...
  if (isLoading) {
    return <div className="text-center py-10">Carregando gráficos...</div>
  }
