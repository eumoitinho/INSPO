...
interface PortalCampaignsProps {
  clientSlug: string
}
...
export default function PortalCampaigns({ clientSlug }: PortalCampaignsProps) {
  ...
  const fetchCampaigns = async () => {
    try {
      ...
      const response = await fetch(`/api/campaigns/${clientSlug}`)
  ...
  if (isLoading) {
    return <div className="text-center py-10">Carregando campanhas...</div>
  }
