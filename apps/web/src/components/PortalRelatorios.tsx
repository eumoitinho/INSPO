...
interface PortalRelatoriosProps {
  clientSlug: string
}
...
export default function PortalRelatorios({ clientSlug }: PortalRelatoriosProps) {
  ...
  const fetchReports = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setError(errorData.error || 'Erro ao carregar relatórios')
      }
    } catch (err) {
